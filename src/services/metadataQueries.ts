 ...
 *
 * All embedded names are escaped via {@link escapeSqlString} to handle
 * apostrophes and similar characters that could otherwise break the literal
 * or allow injection. Names live inside `'...'` literals against RDB$
 * catalog VARCHAR columns, not as identifiers, so escaping (not quoting) is
 * the right tool.
 */
...
export const MetadataQueries = {
    ...
    getTriggers: (tableName?: string) => {
        ...
        `;
        if (tableName) {
            query += ` AND RDB$RELATION_NAME = '${esc(tableName)}'`;
        }
    ...
    getViewSource: (name: string) => `
        ...
        FROM RDB$RELATIONS
        WHERE RDB$RELATION_NAME = '${esc(name)}'
    `,
    ...
    getTriggerSource: (name: string) => `
        ...
        FROM RDB$TRIGGERS
        WHERE RDB$TRIGGER_NAME = '${esc(name)}'
    `,

    getProcedureSource: (name: string) => `
        SELECT RDB$PROCEDURE_SOURCE
        FROM RDB$PROCEDURES
        WHERE RDB$PROCEDURE_NAME = '${esc(name)}'
    `,
    ...
    getProcedureParameters: (procName: string, type: number) => `
        SELECT p.RDB$PARAMETER_NAME, f.RDB$FIELD_TYPE, f.RDB$FIELD_LENGTH, f.RDB$FIELD_PRECISION, f.RDB$FIELD_SCALE, f.RDB$FIELD_SUB_TYPE,
               CAST(p.RDB$DEFAULT_SOURCE AS VARCHAR(255)) AS RDB$DEFAULT_SOURCE,
               CAST(f.RDB$DEFAULT_SOURCE AS VARCHAR(255)) AS DOMAIN_DEFAULT
        FROM RDB$PROCEDURE_PARAMETERS p
        LEFT JOIN RDB$FIELDS f ON p.RDB$FIELD_SOURCE = f.RDB$FIELD_NAME
        WHERE p.RDB$PROCEDURE_NAME = '${esc(procName)}' AND p.RDB$PARAMETER_TYPE = ${type}
    ...
    getTableFields: (name: string) => `
        ...
        WHERE rf.RDB$RELATION_NAME = '${esc(name)}'
    ...
    getTableColumnsDetailed: (tableName: string) => `
        ...
        FROM RDB$RELATION_FIELDS rf
        JOIN RDB$FIELDS f ON rf.RDB$FIELD_SOURCE = f.RDB$FIELD_NAME
        WHERE rf.RDB$RELATION_NAME = '${esc(tableName)}'
        ORDER BY rf.RDB$FIELD_POSITION
    `,
    ...
    getPrimaryKeyColumns: (tableName: string) => `
        ...
        WHERE rc.RDB$RELATION_NAME = '${esc(tableName)}'
    ...
    getTableDependencies: (tableName: string) => `
        ...
        WHERE d.RDB$DEPENDED_ON_NAME = '${esc(tableName)}'
    ...
    getObjectPermissions: (objectName: string, objectType: number) => `
        ...
        WHERE RDB$RELATION_NAME = '${esc(objectName)}'
    ...
    getIndexInfo: (indexName: string) => `
        ...
        FROM RDB$INDICES
        WHERE RDB$INDEX_NAME = '${esc(indexName)}'
    `,
